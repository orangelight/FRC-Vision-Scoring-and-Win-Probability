...
timedata <- dbGetQuery(con, "SELECT * FROM match_table")
outcomes <- read.csv('C:\\Users\\darkd\\Documents\\ScoreProject\\scoreoutcomes.csv')
outcomes <- outcomes[outcomes$outcome!= " ",]
modelList <- list()
for(i in 1:150){
  train_data = merge(x = timedata[timedata$time==i,c(2, 4,5,6, 7, 8, 9)], y = outcomes, by = "match_key", all.x = TRUE)
...
match_key <- '2018mitvc_qf4m2'
match42 = timedata[timedata$match_key==match_key,c(2,4,5,6,7,8,9)]
...
for(i in 150:1) {
  ...
  #curve(predict(modelList[[i]], newdata=data.frame(difference=x), type = "response")+1,add=TRUE)
  #abline(h=1.5)
  predData<- c(predData, predict(modelList[[i]], newdata=match42[match42$time==i, c(1,5,6,7,8)], type = "response"))
}
plot(-1,-1, xlim=c(1,150), ylim=c(0,1), title(main=match_key), xlab="Time(s)", ylab="Probability red wins")
lines(predData)
